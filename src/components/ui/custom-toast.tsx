...
export function CustomToast({ toast, onClose }: ToastProps) {
  ...
  return (
    <div className={`flex items-start gap-3 p-4 rounded-xl border border-l-4 border-border shadow-sm max-w-none md:max-w-sm w-full animate-in slide-in-from-bottom-3 duration-200 ${current.bg}`}>
...
export function ToastContainer({ toasts, onClose }: { toasts: Toast[]; onClose: (id: string) => void }) {
  ...
  return (
    <div className="fixed bottom-4 left-4 right-4 md:left-auto md:right-6 md:bottom-6 z-50 flex flex-col gap-3 max-w-none md:max-w-sm pointer-events-none">
      {toasts.map((toast) => (
